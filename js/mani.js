...
function getCity(q){
    xhr.open('get',`https://api.weatherapi.com/v1/forecast.json?key=fab304cd7daf4049ac7143320240301&q=${q}&days=33`);

    xhr.addEventListener('readystatechange',function(){
    if(xhr.readyState == 4){
        var city = '';
        city += `
        <div class="myFont fs-4">${JSON.parse(xhr.response).location.name}</div>
        <div class="degree">
        <div class="num">${Math.round(Number((JSON.parse(xhr.response).current.temp_c)))}<sup>o</sup>C</div>
        <div class="forecast-icon">
            <img src="https:${JSON.parse(xhr.response).current.condition.icon}" alt="" width=90>
        </div>
        <h6 class="myFont text-info mx-3">${JSON.parse(xhr.response).current.condition.text}</h6>	
        </div>
        <i class="fa-solid fa-umbrella px-1"></i><span>${JSON.parse(xhr.response).current.humidity}%</span>
        <i class="fa-solid fa-wind px-1"></i><span>${JSON.parse(xhr.response).current.wind_kph} km/h<span>
        <i class="fa-regular fa-compass ps-4 pe-1"></i><span>East</span>
        `;
        var nextDays = '';
        nextDays += `
        <div class="forecast-icon">
        <img src="https:${JSON.parse(xhr.response).forecast.forecastday[0].day.condition.icon}" alt="" width=48>
        </div>
        <div class="degree">${Math.round(JSON.parse(xhr.response).forecast.forecastday[0].day.maxtemp_c)}<sup>o</sup>C</div>
        <small>${Math.round(JSON.parse(xhr.response).forecast.forecastday[0].day.mintemp_c)}<sup>o</sup></small><i class="fa-solid fa-temperature-three-quarters px-1"></i>
        <h6 class="myFont text-info mx-3 my-3 fs-5">${JSON.parse(xhr.response).forecast.forecastday[0].day.condition.text}</h6>
        `
        var dayAfterNext = ``;

        dayAfterNext +=`
        <div class="forecast-icon">
        <img src="https:${JSON.parse(xhr.response).forecast.forecastday[1].day.condition.icon}" alt="" width=48>
        </div>
         <div class="degree">${Math.round(JSON.parse(xhr.response).forecast.forecastday[1].day.maxtemp_c)}<sup>o</sup>C</div>
         <small>${Math.round(JSON.parse(xhr.response).forecast.forecastday[1].day.mintemp_c)}<sup>o</sup></small><i class="fa-solid fa-temperature-three-quarters px-1"></i>
        <h6 class="myFont text-info mx-3 my-3 fs-5">${JSON.parse(xhr.response).forecast.forecastday[1].day.condition.text}</h6>
